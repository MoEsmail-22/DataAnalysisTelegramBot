...
const HANDLER_DEADLINE_MS = Number.parseInt(process.env.WEBHOOK_HANDLER_WAIT_MS || '55000', 10);
...
module.exports = async function handler(req, res) {
  ...
  console.log(`[webhook] update ${updateId} from ${fromId} text="${textPreview}"`);
  ...
  try {
    ...
    console.log(`[webhook] update ${updateId} done in ${Date.now() - tStart}ms`);
    return res.status(200).json({ ok: true });
  } catch (error) {
    console.error(`[webhook] update ${updateId} FAILED in ${Date.now() - tStart}ms:`, error);
